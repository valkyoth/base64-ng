...
- Runtime-dispatched std `x86`/`x86_64` SSSE3/SSE4.1 fixed-block strict decode
  in the `1.3.0` working line, limited to Standard and URL-safe alphabets after
...
- Additional SIMD decode acceleration beyond SSSE3/SSE4.1 strict Standard and
  URL-safe decode only after separate `1.3.0` admission evidence is complete.
  The frozen first scope remains padded and unpadded strict decode only: no
  line wrapping, no legacy whitespace, no custom alphabets, no bcrypt/crypt
  profiles, and no constant-time-oriented secret decode. Default builds,
  unsupported runtime CPUs, `no_std`, and all out-of-scope decode surfaces
  remain scalar.
