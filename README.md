...
| RISC-V RVV 1.0 encode/decode | Complete exact-profile backend | QEMU VLEN 128/256 fallback/direct evidence plus native Banana Pi BPI-F3 SpacemiT X60 VLEN 256 correctness, signal/thread, ABI, cleanup, and performance evidence; the release gate requires an exact integrated-source bundle | `admitted` only for the exact Linux/X60 identity from 384 raw encode bytes and 1024 encoded strict-decode bytes; all other RISC-V stays scalar | Not independently verified |
