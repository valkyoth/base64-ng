...
`base64-ng` is intentionally scalar-only in the `0.10.0` release. The crate
uses `#![deny(unsafe_code)]` and permits reviewed
...
- The `0.10.0` release remains scalar-only. Future accelerated dispatch still
  requires the SIMD admission manifest, scalar differential tests, fuzz
  evidence, unsafe inventory, architecture evidence, benchmark evidence, and
  release wording to be updated together.
