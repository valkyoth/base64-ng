...
The first two are selections for qualifying ordinary inputs. Small inputs,
unsupported alphabets, and other documented fallback shapes can still execute
scalar code. Tests therefore run qualifying Standard inputs through test-only
execution counters and compare the observed backend with the corresponding
report. Commit 24 adds forced backend KATs and quarantine. Ordinary reports now
identify `scalar-fixed`, `never-run`, `testing`, `healthy`, `quarantined`, or
