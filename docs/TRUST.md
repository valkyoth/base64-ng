...
| Active backend | Scalar by default; std x86/x86_64 AVX2 encode preferred, then SSSE3/SSE4.1 encode, when `simd` is enabled and runtime CPU probing passes | `runtime::backend_report()` tests |
| SIMD status | AVX2 and SSSE3/SSE4.1 encode admitted for Standard and URL-safe alphabet families; AVX-512, NEON, wasm, custom-alphabet, in-place, and decode acceleration remain prototype-only or scalar | `docs/SIMD.md` |
