...
| Release-evidence authenticity | The final evidence index is signed in a dedicated SSH namespace by the exact pinned release principal/key; reuse verifies the signature, every retained campaign hash, QEMU source/pass markers, and an immutable product tree. Executable release-policy files are never reuse-eligible. | `scripts/sign-release-evidence.sh`, `scripts/verify-release-evidence-signature.sh`, `scripts/evidence-equivalence.py`, `security/evidence-reuse-allowlist.txt` |
