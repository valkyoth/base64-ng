...
| Bytes integration | Fragmented `Buf` input feeds the shared incremental core without coalescing; owned `Bytes` results are transactional; arbitrary `BufMut` states report committed prefixes and latch downstream panics | `scripts/check-2.0-bytes.sh`, `docs/2.0_BYTES_INTEGRATION.md` |
| Tokio async readers | Read-all helpers and fixed-buffer `AsyncRead` adapters use the shared 2.0 core; exact readers preserve adjacent frames; ordinary delivered plaintext is irrevocable | `scripts/check-2.0-tokio-readers.sh`, `docs/2.0_TOKIO_ASYNC_READ.md` |
| Tokio async writers | Fixed-buffer `AsyncWrite` adapters use shared 2.0 state; accepted input and committed output are distinct; queued output survives backpressure and cancellation while the adapter remains alive | `scripts/check-2.0-tokio-writers.sh`, `docs/2.0_TOKIO_ASYNC_WRITE.md` |
| Serde integration | Compatibility modules use validated 2.0 specifications; bounded ordinary fields avoid heap output; secret fields begin fixed-work processing only after format delivery and return wiping storage with opaque errors | `scripts/check-2.0-serde.sh`, `docs/2.0_SERDE_INTEGRATION.md` |
