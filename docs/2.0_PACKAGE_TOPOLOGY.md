...
`secrets` and `checked-backend` are inert reservations during early numbered
commits. Ordinary type layout and auto traits must already remain invariant
when those names are enabled. Their production behavior arrives only at the
numbered commits identified in the release plan.
