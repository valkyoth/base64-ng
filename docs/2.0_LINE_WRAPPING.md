...
Status: Commit 7 internal wrapping-policy contract.

Commit 7 replaces the representable-invalid 1.x wrapping shape with a private
2.0 value whose line width is a `NonZeroUsize`. The module remains private
while the package version is `1.3.9`; later operation commits consume this
value before the complete 2.0 API is exposed.
...
The value is `Copy`, `Send`, `Sync`, `no_std`, and allocation-free. Checked
...
protocol companions. Commit 17 later combines body wrapping with explicitly
...
Their 2.0 replacement is one fallible `try_new` constructor plus immutable
accessors. Existing `LineWrap::MIME`, `LineWrap::PEM`, and
