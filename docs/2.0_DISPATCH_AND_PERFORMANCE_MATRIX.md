...
and checksums. `scripts/validate-neon-admission-bundle.py` independently checks
the exact inventory, manifest schema, source ancestry, runtime-source identity,
host class, checksums, and performance policy. Apple Silicon and server Linux
bundles must remain distinct and must name the same source commit.
