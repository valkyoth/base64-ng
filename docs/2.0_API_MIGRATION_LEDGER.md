...
| `base64-ng-sanitization` | Retained; Commit 40 adds 2.0 `Base64<S>` protected two-allocation fill, removes degraded compatibility defaults, and keeps core `decode_assured` as the provider-quarantined no-copy route |
...
prefix progress for arbitrary `BufMut`, while owned `Bytes` transforms remain
transactional and do not coalesce fragmented input.
