...
runtime codecs, ordinary `EncodedArray` and `DecodedArray` storage without
drop-time cleanup, and a separate non-Clone redacted `SecretArray` that wipes
its full capacity. Const malformed-input and sizing failures remain typed
unless a caller deliberately promotes them to const-evaluation panics.
...
| legacy whitespace-tolerant decode | Use the exact `web::FORGIVING` algorithm only for web strings; use separately named `compat` policies for non-web padding/trailing-bit interoperability |
