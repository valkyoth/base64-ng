...
| CWE-208 Observable Timing Discrepancy | Sensitive comparison exits early on the first different byte | `SecretBuffer`, `EncodedBuffer`, `DecodedBuffer`, and their explicit comparison helpers use dependency-free constant-time-oriented comparison for equal-length inputs. |
