...
blocks, SSSE3/SSE4.1 for full 16-byte encoded blocks, and std aarch64 NEON
for full 16-byte encoded blocks after scalar whole-input validation; every
other decode backend and the `base64_ng::ct` constant-time-oriented decode
path remain scalar unless a separate formal side-channel evidence package
admits otherwise.
