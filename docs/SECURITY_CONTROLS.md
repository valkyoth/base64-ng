...
| CWE-208 Observable Timing Discrepancy | Secret comparison exits early on the first different byte | `SecretBuffer::constant_time_eq` and `SecretBuffer` equality use dependency-free constant-time-oriented comparison for equal-length inputs. |
