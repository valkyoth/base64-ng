...
| CWE-226 Sensitive Information in Resource Not Removed Before Reuse | Caller-owned output buffers or crate-owned staging buffers retain partial sensitive data | Clear-tail APIs, stream cleanup, `EncodedBuffer`, and `SecretBuffer` provide best-effort initialized-byte and spare-capacity cleanup. |
...
| CWE-208 Observable Timing Discrepancy | Sensitive comparison exits early on the first different byte | `SecretBuffer`, `EncodedBuffer`, and their explicit comparison helpers use dependency-free constant-time-oriented comparison for equal-length inputs. |
