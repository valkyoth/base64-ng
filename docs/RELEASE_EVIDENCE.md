...
- no-alloc portability smoke crate checks for stack-backed encode/decode,
  ...
  constant-time-oriented in-place decode, custom alphabet/profile, recoverable
  length, and stack-buffer state surfaces; the harness also runs host-side unit
  tests before cross-target compile checks
