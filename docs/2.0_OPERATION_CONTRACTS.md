...
An incremental transform is in exactly one state: `Active`, `Complete`, or
`Failed(error)`. Failure is absorbing. Only an explicit `reset` creates a new
message after completion or failure.
...
after the caller has declared end of input. Repeated `finish` is idempotent
only after successful completion. A failed state returns its original failure
until reset so later calls cannot erase the first cause.
...
- Terminal errors classify illegal calls after successful completion.
