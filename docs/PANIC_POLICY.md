...
Nested `src/*/tests.rs` and `src/*/*/tests.rs` modules receive the same check
against their immediate parent module declaration; the Commit 22 assurance and
Commit 23 runtime-report tests cannot inherit the test exemption unless their
parent module keeps the explicit `#[cfg(test)] mod tests;` boundary.
...
- `SecretArrayFrame<N>::new` evaluates an associated const assertion and an
  ...
  is instantiated.
- `SecretArrayEncoder<CAP>::new` applies the same type-level policy to encoded
