...
fn exercise_wrapped_encode<A, const PAD: bool>(
    input: &[u8],
...
{
    let encoded = engine
        .encode_wrapped_vec(input, wrap)
        .expect("wrapped encoded length fits");
    let extra = input.len() % 7 + 1;
    let mut output = vec![0xa5; encoded.len() + extra];
    let written = engine
        .encode_slice_wrapped_clear_tail(input, &mut output, wrap)
        .expect("wrapped encoding into large enough output succeeds");
    assert_eq!(&output[..written], encoded);
    assert!(output[written..].iter().all(|byte| *byte == 0));
    ...
    if !encoded.is_empty() {
        let mut too_small = vec![0xa5; encoded.len() - 1];
        let err = engine
            .encode_slice_wrapped_clear_tail(input, &mut too_small, wrap)
            .expect_err("wrapped encoded output exceeds buffer length");
        assert_eq!(
            err,
            base64_ng::EncodeError::OutputTooSmall {
                required: encoded.len(),
...
fn exercise_wrapped_decode<A, const PAD: bool>(
    input: &[u8],
    engine: base64_ng::Engine<A, PAD>,
    wrap: LineWrap,
) where
    A: base64_ng::Alphabet,
{
    let required = engine.decoded_len_wrapped(input, wrap);
    let mut output = vec![0u8; required.unwrap_or(0)];
    let slice_result = engine.decode_slice_wrapped(input, &mut output, wrap);
    ...
    match (&slice_result, &vec_result) {
        ...
        (slice_result, vec_result) => panic!(
            "decode_slice_wrapped and decode_wrapped_vec disagreed: {slice_result:?} vs {vec_result:?}"
        ),
    }

    let mut clear_tail_output = vec![0xa5; required.unwrap_or(0) + 3];
    let clear_tail_result =
        engine.decode_slice_wrapped_clear_tail(input, &mut clear_tail_output, wrap);
    match (clear_tail_result, vec_result) {
        (Ok(written), Ok(decoded)) => {
            assert_eq!(&clear_tail_output[..written], decoded);
            assert!(
        ...
        (clear_tail_result, vec_result) => panic!(
            "decode_slice_wrapped_clear_tail and decode_wrapped_vec disagreed: {clear_tail_result:?} vs {vec_result:?}"
        ),
    }

    if let Ok(decoded) = engine.decode_wrapped_vec(input, wrap)
        && !decoded.is_empty()
    {
        let mut too_small = vec![0xa5; decoded.len() - 1];
        let err = engine
            .decode_slice_wrapped_clear_tail(input, &mut too_small, wrap)
            .expect_err("wrapped decoded output exceeds buffer length");
        assert_eq!(
            err,
            base64_ng::DecodeError::OutputTooSmall {
                required: decoded.len(),
