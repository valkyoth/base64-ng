...
signing_key="${BASE64_NG_EVIDENCE_SIGNING_KEY:-}"

case "$signing_key" in
    *.pub) signing_key="${signing_key%.pub}" ;;
esac
if [ -z "$signing_key" ] || [ ! -f "$signing_key" ]; then
    echo "release evidence signing: set BASE64_NG_EVIDENCE_SIGNING_KEY to the dedicated evidence-only private SSH key" >&2
    exit 1
fi
if [ ! -s "$manifest" ]; then
