...
for function in decode_16_bytes_ssse3_sse41 decode_32_bytes_avx2; do
...
for required in decode_standard decode_url_safe decode_slice_avx2 decode_slice_ssse3_sse41 decode_backend::decode_checked; do
...
if [ "${BASE64_NG_RUN_COMMIT27_PERF:-0}" = "1" ]; then
    evidence="${BASE64_NG_COMMIT27_PERF_FILE:-target/release-evidence/commit-27-x86-decode.csv}"
    ...
    echo "2.0 x86 decode: performance run skipped; set BASE64_NG_RUN_COMMIT27_PERF=1"
fi
