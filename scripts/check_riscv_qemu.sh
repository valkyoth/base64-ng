...
mkdir -p "$evidence_dir"
{
    ...
    echo "public_dispatch=scalar-because-qemu-is-not-the-exact-x60-profile"
    echo "evidence_scope=scalar fallback plus direct RVV encode/decode candidate correctness"
    echo "not_evidence_for=exact-profile production execution, real hardware correctness, performance, timing, or ABI preservation"
    echo "hardware_status=production admission requires the exact native Linux SpacemiT X60 profile"
