...

{
    ...
    echo "runtime_dispatch=avx512-vbmi-encode-then-avx2-encode-then-ssse3-sse4.1-encode-when-supported-else-scalar"
    echo "active_backend_admitted=avx512-vbmi-or-avx2-or-ssse3-sse4.1-encode"
    ...
    echo
    echo "interpretation:"
    echo "This evidence records runtime backend reporting, admitted AVX-512 VBMI, AVX2, or SSSE3/SSE4.1 encode dispatch when supported, and remaining inactive SIMD prototype scalar-equivalence results for this machine."
    echo "NEON and wasm results are prototype evidence; they are not active backend admission evidence."
