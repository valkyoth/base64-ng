...

{
    ...
    echo "runtime_dispatch=avx2-encode-then-ssse3-sse4.1-encode-when-supported-else-scalar"
    echo "active_backend_admitted=avx2-or-ssse3-sse4.1-encode"
    ...
    echo
    echo "interpretation:"
    echo "This evidence records runtime backend reporting, admitted AVX2 or SSSE3/SSE4.1 encode dispatch when supported, and remaining inactive SIMD prototype scalar-equivalence results for this machine."
    echo "AVX-512, NEON, and wasm results are prototype evidence; they are not active backend admission evidence."
    echo "The admitted x86 AVX2 and SSSE3/SSE4.1 paths exercise real fixed-block vector encode logic for Standard and URL-safe alphabets when the required CPU feature bundle is available."
